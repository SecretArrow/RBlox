...
func _place_rainbow_arch(game: Node) -> void:
        ...
        for i in range(seg):
                ...
                _place_shot_block(
                        wm, root, "arch%d" % i, "box", pos, Vector3(1.4, 1.4, 1.4),
                        colors[i % colors.size()], "plastic"
                )
        _place_shot_block(
                wm, root, "stage", "box", Vector3(16.0, 0.1, 8.0),
                Vector3(16.0, 0.2, 5.0), "#37474f", "plastic"
        )
        _place_shot_block(
                wm, root, "pillar_l", "cylinder", Vector3(9.2, 2.0, 8.0),
                Vector3(0.8, 4.0, 0.8), "#ffffff", "neon"
        )
        _place_shot_block(
                wm, root, "pillar_r", "cylinder", Vector3(22.8, 2.0, 8.0),
                Vector3(0.8, 4.0, 0.8), "#ffffff", "neon"
        )


func _place_shot_block(
        wm: GDScript, root: Node3D, id: String, shape: String, pos: Vector3,
        size: Vector3, color: String, mat: String
) -> void:
        wm.call("place_block", root, {
                "id": id, "shape": shape,
                "pos": [pos.x, pos.y, pos.z], "size": [size.x, size.y, size.z],
                "color": color, "mat": mat, "anchored": true
        })
