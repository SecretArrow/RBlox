...
const COMMANDS: Array[String] = ["move_forward", "turn_left", "turn_right", "jump",
		"wait", "repeat_n", "if_player_near", "set_color", "say"]
const ARGS := {"wait": "1.0", "repeat_n": "2", "if_player_near": "4.0",
		"set_color": "#e0453a", "say": "Halo"}
const MAX_REPEAT := 10
...
var _program: Array = []
var _running := false; var _pending := ""; var _editing := -1
var _arg_row: HBoxContainer; var _arg_edit: LineEdit; var _list: ItemList
...
func _ready() -> void:
	...
	dim.set_anchors_preset(Control.PRESET_FULL_RECT); add_child(dim)
	...
	_arg_row = HBoxContainer.new(); _arg_row.visible = false
	_arg_row.add_theme_constant_override("separation", 8); box.add_child(_arg_row)
	_arg_edit = LineEdit.new(); _arg_edit.custom_minimum_size = Vector2(0, 48)
	_arg_edit.size_flags_horizontal = Control.SIZE_EXPAND_FILL; _arg_row.add_child(_arg_edit)
	_arg_row.add_child(UiKit.make_button(Locale.t("common_ok"), Vector2(96, 48), true))
	_list = ItemList.new(); _list.custom_minimum_size = Vector2(0, 150)
	_list.size_flags_vertical = Control.SIZE_EXPAND_FILL
	_list.item_selected.connect(_on_item_selected); box.add_child(_list)
	var row := HBoxContainer.new(); row.add_theme_constant_override("separation", 8)
	box.add_child(row)
	for c in [["bc_up", _edit_item.bind(-1, false)], ["bc_down", _edit_item.bind(1, false)],
			["common_delete", _edit_item.bind(0, true)],
			["bc_run", func() -> void: run_script(_target)], ["common_save", _on_save],
			["bc_advanced", _on_advanced], ["common_close", func() -> void: visible = false]]:
...
func _tap_btn(text: String, cb: Callable, size: Vector2 = Vector2(120, 48)) -> Button:
	var b := UiKit.make_button(text, size, false); b.pressed.connect(cb)
	return b
...
func _on_cmd(cmd: String) -> void:
	_pending = cmd; _editing = -1
	if not ARGS.has(cmd):
		_arg_row.visible = false; _append(cmd, ""); return
	_arg_edit.text = str(ARGS[cmd]); _arg_row.visible = true

## Pilih item di list -> edit argumennya lewat baris arg yang sama.
func _on_item_selected(idx: int) -> void:
	if idx < 0 or idx >= _program.size(): return
	var e: Dictionary = _program[idx]
	if not ARGS.has(str(e.get("cmd", ""))):
		_arg_row.visible = false; return
	_editing = idx; _pending = str(e.get("cmd", ""))
	_arg_edit.text = str(e.get("arg", "")); _arg_row.visible = true

func _on_arg_ok() -> void:
	if _pending == "": return
	var cmd := _pending; var arg := _arg_edit.text.strip_edges()
	_arg_row.visible = false
	if _editing >= 0 and _editing < _program.size():
		_program[_editing]["arg"] = arg; _refresh_list()
	else:
		_append(cmd, arg)
	_pending = ""; _editing = -1
...
func _edit_item(dir: int, remove: bool) -> void:
	var i := _sel_index()
	if remove:
		if i >= 0: _program.remove_at(i)
	elif i >= 0 and i + dir >= 0 and i + dir < _program.size():
		var e: Dictionary = _program[i]
		_program.remove_at(i); _program.insert(i + dir, e)
		_list.select(i + dir)
	_refresh_list()
...
func run_script(target: Node3D) -> void:
	if _running or _program.is_empty():
		_toast(Locale.t("bc_running") if _running else Locale.t("bc_empty")); return
	if target == null or not is_instance_valid(target):
		_toast(Locale.t("bc_no_target")); return
	_target = target; _running = true
	# EN: repeat_n -> run the whole program min(n, 10) times (outer loop).
	var passes := 1
	for e in _program:
		if str(e.get("cmd", "")) == "repeat_n":
			var s := str(e.get("arg", "2"))
			passes = clampi(int(s) if s.is_valid_int() else 2, 1, MAX_REPEAT); break
	var stopped := false
	for _p in passes:
		for i in _program.size():
			var cmd := str(_program[i].get("cmd", ""))
			if cmd == "repeat_n": continue
			if cmd == "if_player_near":
				var r := str(_program[i].get("arg", ""))
				if not _player_near(float(r) if r.is_valid_float() else 4.0):
					_toast(Locale.t("bc_far")); stopped = true; break
				continue
			await _exec(_program[i])
		if stopped or not _valid(): break
	_running = false
	if not stopped: _toast(Locale.t("bc_done"))
...
func _exec(e: Dictionary) -> void:
	...
	match str(e.get("cmd", "")):
		"move_forward":
			_target.translate_object_local(Vector3(0, 0, -1)); await t.create_timer(STEP_DELAY).timeout
		"turn_left":
			_target.rotate_y(PI * 0.5); await t.create_timer(STEP_DELAY).timeout
		"turn_right":
			_target.rotate_y(-PI * 0.5); await t.create_timer(STEP_DELAY).timeout
		"jump":  # EN: small hop: +0.6 up, then back down.
			_target.translate(Vector3(0, 0.6, 0)); await t.create_timer(0.3).timeout
			if _valid(): _target.translate(Vector3(0, -0.6, 0))
		"wait":
			var secs := minf(float(arg) if arg.is_valid_float() else 1.0, WAIT_MAX)
			await t.create_timer(maxf(secs, 0.05)).timeout
		"set_color": _apply_color(arg)
		"say": _say(arg)
...
func _on_save() -> void:
	...
	if not meta.is_empty():
		meta["script"] = arr; _target.set_meta("block_data", meta)
	else: _target.set_meta("script", arr)
...
func _on_advanced() -> void:
	if not _valid() or not ResourceLoader.exists(MINI_PATH):
		_toast(Locale.t("bc_no_target") if not _valid() else Locale.t("error_not_impl")); return
...
func _player_near(radius: float) -> bool:
	if not _valid() or get_tree() == null: return false
...
func _apply_color(hex: String) -> void:
	...
	if mi != null:
		...
		var m := (src as StandardMaterial3D).duplicate() if src is StandardMaterial3D \
				else StandardMaterial3D.new()
		m.albedo_color = Color(c); mi.material_override = m
	var meta: Dictionary = _target.get_meta("block_data", {})
	if not meta.is_empty():
		meta["color"] = c; _target.set_meta("block_data", meta)

func _say(text: String) -> void:
	if not _valid() or text == "" or get_tree() == null: return
	var l := Label3D.new(); l.text = text
	l.billboard = BaseMaterial3D.BILLBOARD_ENABLED; l.position = Vector3(0, 1.6, 0)
	_target.add_child(l)
