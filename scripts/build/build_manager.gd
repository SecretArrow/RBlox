...
## terrain, undo/redo 50 langkah, limit 4000 blok, statistik & achievement.
## Sinyal block_changed(data, removed) dipakai game.gd untuk sync multiplayer.
...
func _do_place(hit: Dictionary) -> void:
	...
	if root.get_child_count() >= MAX_BLOCKS:
		_toast_tr("build_limit", "Block limit reached (4000)")
		return
