...
func _build_3d() -> Control:
        ...
        env.background_color = Color("#2c3650")
        ...
        # Yaw -15: berkas cahaya mengarah ke -Z (menembus layar) sehingga sisi
        # depan avatar yang menghadap kamera terterang, bukan backlight.
        sun.rotation_degrees = Vector3(-35, -15, 0)
        sun.light_energy = 1.3
        sv.add_child(sun)
        var cam := Camera3D.new()
        cam.position = Vector3(0, 1.0, 3.6)
...
func _build_right() -> Control:
        ...
        for slot in SLOTS:
                ...
                var fb: Array = SLOT_FB.get(slot, [slot, slot, slot])
                tabs.set_tab_title(tabs.get_tab_count() - 1, _tr(str(fb[0]), str(fb[1]), str(fb[2])))
