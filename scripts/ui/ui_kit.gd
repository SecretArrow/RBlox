...
static func make_button(
        ...
        b.add_theme_stylebox_override(
                "pressed", _button_3d(base.darkened(0.22), true)
        )
        b.add_theme_stylebox_override(
                "disabled", _button_3d(Color(0.45, 0.48, 0.56, 0.5))
        )
        ...
        b.button_up.connect(
                func() -> void:
                        if not is_instance_valid(b):
                                return
                        var tw := b.create_tween()
                        tw.tween_property(b, "scale", Vector2.ONE, 0.12).set_trans(
                                Tween.TRANS_BACK
...
static func make_decor_blocks() -> Control:
        ...
        for s in specs:
                ...
                tw.set_loops()
                tw.tween_property(box, "position:y", -14.0, 2.4 + float(s[2]) * 0.01).set_trans(
                        Tween.TRANS_SINE
                ).set_ease(Tween.EASE_IN_OUT).as_relative()
                tw.tween_property(box, "position:y", 14.0, 2.4 + float(s[2]) * 0.01).set_trans(
                        Tween.TRANS_SINE
                ).set_ease(Tween.EASE_IN_OUT).as_relative()
        return layer
