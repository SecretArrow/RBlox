...
##      update_block_color / flush_all / get_block_count / get_stats /
##      visible_tris.
...
func register_block(
	id: String, xf: Transform3D, shape: String, size: Vector3, color: String, mat: String
) -> void:
	if id == "":
		return
	if _blocks.has(id):
	...
	_blocks[id] = {
		...
		"bucket": bucket,
		"xf": xf,
		"color": BlockLibrary.parse_color(color),
		"shape": shape,
		"mat": mat,
		"size": size,
	}
...
## Estimasi tris yang benar-benar terlihat kamera (status LOD terakhir).
## Dipakai PolyBudget untuk menjaga total scene 100k-200k tris.
func visible_tris() -> int:
	var vis := 0
	for ckey in _chunks.keys():
		var buckets: Dictionary = (_chunks[ckey] as Dictionary)["buckets"]
		for bkey in buckets.keys():
			var b: Dictionary = buckets[bkey]
			var mmi: Variant = b.get("mmi")
			if mmi != null and is_instance_valid(mmi) and (mmi as MultiMeshInstance3D).visible:
				vis += _bucket_tris(bkey, b)
	return vis


## Rekap lengkap beban render: jumlah chunk, instance, dan tris.
func get_stats() -> Dictionary:
	...
	for ckey in _chunks.keys():
		...
		for bkey in buckets.keys():
			var b: Dictionary = buckets[bkey]
			var t := _bucket_tris(bkey, b)
			total_tris += t
			var mmi: Variant = b.get("mmi")
			if mmi != null and is_instance_valid(mmi) and (mmi as MultiMeshInstance3D).visible:
				chunk_vis = true
				vis_inst += (b["ids"] as Array).size()
				vis_tris += t
		if chunk_vis:
			vis_chunks += 1
	return {
		"chunks_total": _chunks.size(),
		"chunks_visible": vis_chunks,
		"instances_visible": vis_inst,
		"tris_visible": vis_tris,
		"tris_total": total_tris,
	}


## Tris satu bucket = tris per bentuk x jumlah instance (multiplier skala
## tidak mengubah jumlah tris). Bentuk tak dikenal dianggap box (12).
func _bucket_tris(bkey: Variant, b: Dictionary) -> int:
	var shape := String(bkey).split("|")[0]
	var per: int = PolyBudget.SHAPE_TRIS.get(shape, 12)
	return per * (b["ids"] as Array).size()
...
func _rebuild_chunk(ckey: Variant) -> void:
	...
	for bkey in buckets.keys():
		var b: Dictionary = buckets[bkey]
		var ids: Array = (b["ids"] as Array).filter(func(i: Variant) -> bool: return _blocks.has(i))
		b["ids"] = ids
		if ids.is_empty():
...
## Catatan PolyBudget: jumlah segmen primitif di bawah sengaja rendah agar
## tiap bentuk tetap di bawah budget lingkungan 500 tris per objek (box 12,
## wedge 8, cylinder 192, sphere 288) - diverifikasi tools/test_budget.gd.
