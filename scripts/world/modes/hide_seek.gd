...
func session_state() -> Dictionary:
        return {
                "countdown": _countdown, "survive": _survive,
                "hiding": _hiding, "caught": _caught, "won": _won,
        }


func restore_session(s: Dictionary) -> void:
        _countdown = float(s.get("countdown", HIDE_TIME))
        _survive = float(s.get("survive", 0.0))
        _hiding = bool(s.get("hiding", true))
        _caught = bool(s.get("caught", false))
        _won = bool(s.get("won", false))
