...
func session_state() -> Dictionary:
        var hits := []
        for f in _fuses:
                hits.append(bool(f.get("hit", false)))
        return {
                "fuse_hits": hits, "count": _count, "escaped": _escaped,
                "has_exit": _has_exit, "exit": [_exit.x, _exit.y, _exit.z],
        }


func restore_session(s: Dictionary) -> void:
        _count = int(s.get("count", 0))
        _escaped = bool(s.get("escaped", false))
        _has_exit = bool(s.get("has_exit", false))
        var e: Variant = s.get("exit", [])
        if e is Array and (e as Array).size() >= 3:
                _exit = Vector3(float(e[0]), float(e[1]), float(e[2]))
        var hits: Variant = s.get("fuse_hits", [])
        if hits is Array:
                for i in range(mini(_fuses.size(), (hits as Array).size())):
                        _fuses[i]["hit"] = bool(hits[i])
