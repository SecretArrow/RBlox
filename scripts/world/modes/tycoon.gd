...
func session_state() -> Dictionary:
        var owned := []
        for pad in _pads:
                owned.append(bool(pad.get("owned", false)))
        return {"cash": _cash, "owned": owned, "plot": _plot}


func restore_session(s: Dictionary) -> void:
        _cash = float(s.get("cash", 0.0))
        _plot = int(s.get("plot", 0))
        var owned: Variant = s.get("owned", [])
        if owned is Array:
                for i in range(mini(_pads.size(), (owned as Array).size())):
                        _pads[i]["owned"] = bool(owned[i])
