...
func _ready() -> void:
	...
	var wj: Dictionary = {
		...
		"meta": {"name": "Scale"}, "settings": {},
		"spawn_points": [[0, 3, 0]], "blocks": [], "props": [],
		"terrain": {"size": 32, "seed": 1, "heights": [], "paint": []}
	}
	var blocks: Array = []
	for i in 12000:
		blocks.append({
			"id": "s%d" % i, "shape": ["box", "box", "box", "sphere", "cylinder", "wedge"][i % 6],
			"pos": [float(i % 23), 1.0 + float((i / 23) % 40), float((i / 23) / 40)],
			"size": [1, 1, 1],
			"color": "#e0453a", "mat": ["plastic", "metal", "wood", "glass", "neon"][i % 5],
			"anchored": true
		})
	wj["blocks"] = blocks
	...
	var ser: Variant = wm.call("serialize_world", (res as Dictionary)["root"] as Node3D, wj.duplicate(true))
	...
	for c in mmr.get_children():
		if c is MultiMeshInstance3D:
			mmi += 1
	print("SCALE_TEST build_ms=%d serialize_ms=%d blocks=%d mmi=%d nodes=%d" % [t1 - t0, t2 - t1, count, mmi, blocks_root.get_child_count()])
